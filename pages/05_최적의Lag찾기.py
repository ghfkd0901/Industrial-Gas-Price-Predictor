...
try:
    ...
    # 선택된 각 원자재 변수 추가 (Secondary Y-axis)
    colors = {'Brent': '#e74c3c', 'TTF': '#2ecc71', 'USD_KRW': '#3498db'}
    
    for var in selected_vars:
        # 토글 스위치 상태에 따라 시차 적용 여부 결정
        current_lag = best_lags[var] if apply_lag else 0
        display_name = f"{var} (Lag {current_lag}M)"
        
        fig_line.add_trace(go.Scatter(
            x=df.index, y=df[var].shift(current_lag), 
            name=display_name,
            line=dict(dash='dot', color=colors[var]),
            yaxis="y2"
        ))

    fig_line.update_layout(
        height=550,
        title=f"<b>{'[시차 적용]' if apply_lag else '[시차 미적용]'}</b> 실시간 추세 정렬 상태",
        xaxis_title="날짜",
        yaxis_title="도매요금 (원)",
        yaxis2=dict(title="원자재 가격 지수", overlaying='y', side='right'),
        hovermode="x unified",
