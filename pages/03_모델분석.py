...
def load_all_data():
    ...
    m_df = pd.DataFrame(raw_master[1:], columns=raw_master[0])
    m_df = m_df.replace('', np.nan).dropna(how='all').dropna(axis=1, how='all')

    date_col = m_df.columns[0]
    m_df[date_col] = pd.to_datetime(m_df[date_col], errors='coerce')
    m_df = m_df.dropna(subset=[date_col]).set_index(date_col)
    for col in m_df.columns:
        m_df[col] = pd.to_numeric(
            m_df[col].astype(str).str.replace(',', ''), errors='coerce'
        )

    # ── gas_price 로드
    ws_gas = sh.worksheet("gas_price")
    raw_gas = ws_gas.get_all_values()
    g_df = pd.DataFrame(raw_gas[1:], columns=raw_gas[0])
    g_df = g_df.replace('', np.nan).dropna(how='all').dropna(axis=1, how='all')

    date_col_g = g_df.columns[0]
    g_df[date_col_g] = pd.to_datetime(g_df[date_col_g], errors='coerce')
    g_df = g_df.dropna(subset=[date_col_g]).set_index(date_col_g)
    g_df.columns = ['Wholesale_Price']
    g_df['Wholesale_Price'] = pd.to_numeric(
        g_df['Wholesale_Price'].astype(str).str.replace(',', ''), errors='coerce'
    )
...
if not selected_features:
    ...
    st.markdown(
        f"📅 **분석 기간:** 2015-01-01 ~ 현재 &nbsp;&nbsp;|&nbsp;&nbsp; "
        f"🎓 **학습 기간:** {train_start_year}년 ~ {train_end_year}년 ({len(X_train)}개월) "
        f"&nbsp;&nbsp; {lag_summary}"
    )
